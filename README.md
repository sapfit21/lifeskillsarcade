...
the stylesheet, the script, this README, and the copies of the markdown under
`downloads/`. You can run it as many times as you like. It always starts clean.
...
- `downloads/`, a copy of every source markdown file, so the download links on
  the pages work. The build refreshes these every time.
...
students. There is no login, no form, no tracking and no network request of any
kind. Keep it that way.
